...
public class Email {
    ...
    public void send(String to, EmailTemplate template, Optional<String> link) throws Exception {
        _logger.log(Level.INFO,  "send email begin");
        if(to.isEmpty() || (template.equals(EmailTemplate.RESET_LINK) && link.isEmpty())) {
        ...
        try {
            html = resetTemplate(link.get());
            text += link.get();
        } catch (NoSuchElementException ex) {
            _logger.log(Level.WARNING, "send email --> link value not avl.");
